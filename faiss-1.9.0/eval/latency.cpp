...
int main(int argc, char **argv) {
    ...
    std::string param3 = argv[3]; // nlist value
    std::string param4 = argv[4]; // k
    std::string param5 = argv[5]; // starting nprobe to speed up computation
    ...
    for (float alpha : alphas) {
        ...
        int test_start_idx = nq - calib_nq;
        ...
        for (int i = test_start_idx; i < nq; ++i) {
            ...
            double t1 = elapsed();
            index->search(1, xi, k, dis.data(), nns.data());
            latencies.push_back((elapsed() - t1) * 1000);
        }

        float average_latency = std::accumulate(latencies.begin(), latencies.end(), 0.0f) / latencies.size();
